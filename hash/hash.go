...
func Compute(hash hash.Hash, data []byte, more ...[]byte) ([]byte, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "could not compute hash value of main data")
	}

	for i, v := range more {
		_, err := hash.Write([]byte(v))
		if err != nil {
			return nil, errors.Wrapf(err, "could not compute hash value for additional data [index: %d]", i)
		}
	}
